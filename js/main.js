...
database.ref('/rooms').once('value').then(function (snapshot) {
    ...
    rooms.forEach(function (index) {
        let images = '';
        index.images.forEach(function (image) {
            images += '<img class="rounded" data-u="image" src=' + image + '/>'
        });
        let html = '                    <div data-p="225.00">\n' +
                                        images +
            '                        <div class="description-room">\n' +
            '                            <button class="all-photos" data-toggle="modal" data-target="#exampleModal">Все фото</button>\n' +
            '                            <h5>Аппартаменты с видом на море • от 1500р. за ночь</h5>\n' +
